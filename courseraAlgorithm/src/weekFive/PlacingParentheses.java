...
public class PlacingParentheses {
	...
	private static int[] getMinAndMax(String exp, int i, int j, int[][] min, int[][] max) {
		...
		for (int index = i; index <= j - 1; index++) {
			...
			long a = eval(min[i][index], min[index + 1][j], oper), b = eval(min[i][index], max[index + 1][j], oper),
					c = eval(max[i][index], min[index + 1][j], oper), d = eval(max[i][index], max[index + 1][j], oper);
