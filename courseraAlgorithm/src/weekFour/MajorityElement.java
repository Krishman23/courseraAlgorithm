...
public class MajorityElement {
	...
	private static int getMajorityElementLinear(int[] a, int left, int right) {
		...
		for (int i = 1; i < a.length - 1; i++) {
			...
			//System.out.println("CountBefore: "+count);System.out.println("maxIB: "+maxIndex);
			if (count == 0) {
				maxIndex = i;
				count = 1;
			} 
			//System.out.println("CountAfte: "+count);
			//System.out.println("maxIA: "+maxIndex);
			//System.out.println();
		}

//System.out.println("a max: "+a[maxIndex]);
		count = 0;
	...
	public static void main(String[] args) {
		...
		int[] a = new int[n];
		for (int i = 0; i < n; i++) {//System.out.println("Enter"+i+": ");
			a[i] = scanner.nextInt();
		}
